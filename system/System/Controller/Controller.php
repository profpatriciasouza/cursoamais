...
class System_Controller extends Template {
    ...
    public function __construct($params = array()) {
        ...
        $this->boot(); //Não implementar em controles funcionais
        $this->init();

        $metodo = $this->getMetodo();

        $this->retornoAcao = $this->$metodo();

        if ($this->render !== false) {
            if (strtoupper($this->render) === "JSON") {
                header('Content-Type: application/json');
                echo $this->retornoAcao;
            } else {
                $this->getFile(strtolower(Map::$modulo) . "/" . $this->render . ".php");
            }
        }
        $this->finish(); //Não implementar em controles funcionais
        $this->end();
    }
